...
| 07-31 | Day 24 | 模块 11 | 1/1 | - | ~2.5h | 视频 I/O：元数据 + 每5帧标注 + 单趟写视频 + 8 项修复（内存 O(1)） |
| 08-04 | Day 25 | 模块 11 | 1/1 | 100% (5/5) | ~4h | 素材体检（相机稳定/手干扰）→ MOG2 追踪 6 函数实现 → 4 bug 修复 → 27 组参数扫描调优（5-10s 基准 mean 4.2→1.0）→ 端到端视频验证 + 模块 11 测验 |
| 08-05 | Day 26 | 模块 12 | 1/1 | - | ~2h | 多 ROI 分析：3 ROI 配置 → 检测器切 HSV 红球分割（Canny 无法分离低对比度球）→ 相对面积阈值调优 → 四面板 + JSON 交付 + code review 2 项修复 |

---

## 每日学习日志

### Day 26 (2026-08-05) — 模块 12 概念 A+B：多 ROI 分析 + 数据打包

**完成事项：**
- [x] 脚本验证：frame 100 灰度 Canny 检测完全找不到球——球体 std≈26 低对比度，边缘碎片与边框连成巨型轮廓，Canny×闭运算核 12 组网格调参均无效
- [x] 检测器切换：灰度 Canny → HSV 红色分割（双 hue 波段 0-10 ∪ 170-180 + 15×15 闭运算 + 3×3 开运算）
- [x] 相对面积阈值：MIN_AREA 绝对像素 → MIN_AREA_FRACTION=0.15%（ROI 面积比例），panorama 从 2 物体变 1（滤掉 0.075% 的角落红块，保留 0.25% 的球）
- [x] 补 m00==0 除零防护（degenerate contour 跳过）
- [x] code review：2 项修复（相对阈值替代绝对阈值 / 场景红块在 panorama 属正常现象并注明）
- [x] 跨帧验证：frame 50-400 球在 panorama 6/7 检出；work_area 坐标映射正确（ROI 局部 + 偏移 = 绝对坐标）
- [x] 交付：四面板 PNG + 终端表 + JSON（frame 100 球框 (625,736,93,84) vs 真实 (624,735,94,85)，误差 1px）
- [x] 7 个 commit（feat×3 + style×2 + tune×2）+ code-reviewer 审核

**关键发现：**
- 检测器必须匹配目标特征通道：红色球体在灰度里 std≈26 没有可分信号，Canny 阈值再怎么扫也分离不出；HSV 色域一步到位——这是"调参救不了选错的特征"
- 绝对阈值对多 ROI 是错误抽象：panorama 2M px vs inlet 130K px 面积差 16 倍，同一绝对像素数在不同窗口含义完全不同；相对 ROI 面积阈值让同一检测逻辑自动缩放
- 球体掩码面积逐帧波动 4-5 倍（运动模糊/遮挡）——静态阈值无法覆盖全视频，frame 300 掉球是单帧分析的固有局限，已在 docstring 注明（帧间追踪是 Day 25 MOG2 的领域）

**错误分析：**
- K：把 Day 20 的灰度 Canny 管线直接搬到多 ROI 视频分析——Canny 适合高对比形状边界，而本素材目标是低对比度红球；选特征通道前应先验证目标可分性

**复盘三问：**
1. 为什么 Canny 调参救不了球？灰度中球与背景 std≈26 对比太弱，边缘碎片化，闭运算后碎片与背景边缘连成一体，RETR_EXTERNAL 把全帧包成 1 个 1.9M px 的巨型轮廓——不是参数问题，是特征通道（灰度）里根本没有可分信号。
2. 相对阈值为什么是多 ROI 的正解？各 ROI 面积差 16 倍，绝对像素阈值在不同窗口含义完全不同；MIN_AREA_FRACTION（ROI 面积分数）使"同一管线、不同尺度"自洽，还顺带滤掉相对过小的角落红块。
3. 和最终项目的关系？ROI 配置 → 独立分析 → 汇总 JSON 正是数据管道的骨架；检测器是可插拔模块——本素材目标物体用 HSV，换目标只需换特征通道，管道架构不变。
...
**完成事项：**
- [x] 选择题 Q2-Q4 全对（Q1 错了 Otsu vs 自适应阈值场景）
- [x] 代码补全 Q5-Q6 全对（arcLength 函数名 + Pipeline 步骤顺序）
- [x] 实战题 Q7：独立合成图 Pipeline 脚本 `day_21_stage3.py`
- [x] 6 步 Pipeline 完整运行：gray → blur → Canny → morph_close → contours → geometry
- [x] 合成图 3 类形状全部正确分类：Triangle(12334) / Circle(11326) / Rectangle(10190)
- [x] 2×3 管线中间结果图保存到 `data/processed/day_21_pipeline.png`

**关键发现：**
- Otsu 仍是全局阈值，光照不均场景光靠 Otsu 不够——需要自适应阈值做局部处理
- 合成图三种形状的圆度：Triangle=0.555 / Rectangle=0.794 / Circle=0.899——理论值对照确认
- 独立脚本零外部依赖是最佳测试实践——考试场景下避免文件找不到的隐患
...
### Day 16 (2026-07-02) — 模块 7 概念 B+C：形态学操作（腐蚀/膨胀/开/闭/梯度）

**完成事项：**
- [x] create_test_image：合成含噪二值图（3 矩形 + 1 圆 + 竖线两处断口 + 横线缺口 + salt/pepper 噪声）
- [x] get_kernel：RECT / ELLIPSE / CROSS 三种结构元素
- [x] apply_morphology：6 操作对比（original / erode / dilate / opening / closing / gradient）
- [x] build_morphology_grid：3×6 matplotlib 网格图，按 kernel 尺寸 3/5/7 三组对比（headless 替代 Trackbar）
- [x] analyze_element_shapes：ASCII 核可视化 + 结构元素选型指南 + 伪影说明
- [x] 3 处合成图修复：竖线被大圆覆盖 / 横线缺口被竖线填满 / pepper 噪声太稀疏
- [x] lint + format 全绿 + 5 个 commit push
- [x] 模块 7 测验：4.5/5 (90%)

**关键发现：**
- 腐蚀 = 白色区域变小（黑色吞噬白色），膨胀 = 白色区域变大（白色吞噬黑色）——迭代次数越多方向性越明显（测验 Q4 的判断陷阱）
- 开运算（先腐蚀后膨胀）= 去小白噪点 + 断开细连接；闭运算（先膨胀后腐蚀）= 填小黑洞 + 连接断线
- MORPH_GRADIENT = 膨胀 - 腐蚀 = 物体外轮廓带
- RECT 核在圆形边界产生直角伪影，ELLIPSE 核更适合圆形物体（测验 Q3）
- headless 环境下 createTrackbar 不可用，用"3×6 网格图批量对比"实现同样的调参观察效果

**错误分析：**
- [A] 模块 7 测验 Q5：二值化应放在透视矫正**之后**而非之前——矫正后文字笔画方向正、文档区域规则，自适应二值化效果才稳定（场景应用判断错）

**复盘三问：**
1. 腐蚀和膨胀对白色区域的影响方向？腐蚀让白色瘦一圈（黑色吞噬白色），膨胀让白色胖一圈（白色吞噬黑色）——iteration 越多效果越强，这是 Q4 判断的核心。
2. 开运算后大物体尺寸变小了怎么办？腐蚀阶段去掉的边界，膨胀阶段无法完全恢复——若必须保持尺寸，改用更小的 kernel 或只处理噪点区域；多数场景尺寸微缩是去噪的合理代价。
3. 圆形硬币 vs 方形二维码选哪种结构元素？硬币用 ELLIPSE（边界贴合、无直角伪影），二维码用 RECT（方形核与方形边界对齐，不破坏角点）。
...
| 07-23 | Otsu（全局阈值）vs 自适应阈值（局部阈值）适用场景混淆 | 测试 3 | 1 | Day 22 特征模块前做一次二值化策略对比复习 | 待改善 |
| 07-31 | 计数逻辑缩进层级（saved 应在采样分支内）| 模块 11 | 1 | 后续 Pipeline 计数时注意分支层级 | 待观察 |
| 07-31 | 视频元数据尺寸（声明 vs 实际帧 shape）| 模块 11 | 0 | Day 26 多 ROI 分析中强化 | 待观察 |
| 08-04 | VideoWriter 尺寸必须与写入帧一致（原始帧 vs 三面板）| 模块 11 | 1 | writer 用 panel.shape 初始化；Day 27 管道重申 | 已改善 |
| 08-05 | 检测器与目标特征通道匹配（灰度 Canny vs HSV 色域）| 模块 12 | 1 | 选检测器前先验证目标可分性；换目标物体时换特征通道 | 已改善 |
| 08-04 | 迭代 dict 时删除元素（Python RuntimeError）| 模块 11 | 1 | list(keys()) 快照后再删；轨迹更新循环已修 | 已改善 |
...
| 测试 3 (Day 21) | 07-23 | 3/4 | 2/2 | 1/1 | 19/20 (95%) | Q1 [K] Otsu vs 自适应阈值场景 |
...
|:---|:---|:---|:---|
| Day 24 | 视频 I/O 基础 | - | - |
| Day 25 | 运动检测追踪（MOG2 + 贪心最近邻轨迹关联） | 相机不稳致 mask 泛白；VideoWriter 尺寸不匹配致写帧失败；MIN_AREA 太松碎片泛滥 | 素材体检先行 + writer 按 panel 尺寸 + MIN_AREA 200→8000 / kernel 9×9 调优 |
| Day 26 | 多 ROI 分析 | Canny 无法分离低对比度红球 | 检测器切换 HSV 红球分割 + MIN_AREA 改 ROI 面积相对阈值 |
| Day 27 | 数据管道 v0.1 | - | - |
| Day 28 | 阶段测试 4 + 框架搭建 | - | - |
| Day 29 | 全流程整合 | - | - |
| Day 30 | 完善 + 演示 | - | - |
...
Week 2 图像变换:     7/7 天  ✓
Week 3 图像分析:     7/7 天  ✓
Week 4 进阶+项目:    5/9 天  (含 Day 29-30 项目冲刺)
----------------------------------------------
总进度:             26/30 天
```
